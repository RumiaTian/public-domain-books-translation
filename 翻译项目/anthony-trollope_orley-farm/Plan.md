...
- **项目名称**：anthony-trollope_orley-farm（《奥利农场》，安东尼·特罗洛普长篇小说）
- **源文目录**：原文/
- **译文目录**：译文/
...
```
file,size_kb,status
01-the-commencement-of-the-great-orley-farm-case.md,14.2,todo
02-lady-mason-and-her-son.md,17.9,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `01-the-commencement-of-the-great-orley-farm-case.md` |
...
> **谁执行**由 `项目说明.md` 的「内容形态」字段决定：本项目为长篇，走**委派模式**（子代理逐篇执行本流程）。
...
全书 80 章，文件名两位数字前缀（01-… 至 80-…）保证阅读顺序，清单以 `translation_queue.csv` 为准（80 行）。
