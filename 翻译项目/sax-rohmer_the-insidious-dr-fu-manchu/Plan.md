# 翻译计划：The Insidious Dr. Fu Manchu

---

## 本计划信息

- **项目名称**：sax-rohmer_the-insidious-dr-fu-manchu（《阴险的傅满洲博士》）
- **源文目录**：原文/
- **译文目录**：译文/
...
| `原文/*.md` | 源文（30 章，01-i.md … 30-xxx.md，数字前缀=阅读顺序） | 不改 |
...
> 本项目内容形态为**长篇**，执行模式为**委派**：主 agent 只当调度员，把每章的 9 步下沉到一次性子代理执行（并发 ≤ 3），调度规则见 `WORKFLOW.md`「委派模式调度循环」。
...
- 长文（源文 >50KB）分段处理见通用翻译引擎第七节（本项目单章 5–21KB，无需分段）。

### 5. 自检
- 完整性：源文每一段都在译文 Original 块中出现，末尾无截断。
- 块对称：Original/Chinese 数量相等，每块内段数对应。
- 标题格式：全部「英文 / 中文」合并（罗马数字章号照搬）。
- 术语一致：抽查 5 个术语词（如傅满洲、奈兰·史密斯、卡拉曼娜、韦茅斯、扎亚特之吻），全文译法统一。
- 不满足则改到满足。
...
共 30 章（罗马数字 I–XXX），详见 `translation_queue.csv`；文件名两位数字前缀保证顺序。
