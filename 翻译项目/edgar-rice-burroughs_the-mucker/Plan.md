...
- **项目名称**：edgar-rice-burroughs_the-mucker（《恶棍》The Mucker，Edgar Rice Burroughs，1914）
- **源文目录**：原文/
- **译文目录**：译文/
...
```
file,size_kb,status
01-billy-byrne.md,16.2,todo
02-shanghaied.md,15.8,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `01-billy-byrne.md` |
...
> **谁执行**由 `项目说明.md` 的「内容形态」字段决定：本项目为长篇 → **委派模式**，子代理逐篇执行以下 9 步。
...
第一部（Part I）：01–18 章，芝加哥→「半月号」→荒岛→旧金山。
第二部（Part II）：19–35 章，审判逃亡→墨西哥→团圆。

完整清单见 `translation_queue.csv`（35 章，全 todo）。
