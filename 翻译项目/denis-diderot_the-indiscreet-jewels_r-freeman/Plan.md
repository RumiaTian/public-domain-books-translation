...
- **项目名称**：denis-diderot_the-indiscreet-jewels_r-freeman（《泄密的珠宝》，狄德罗著，R. Freeman 英译本）
- **源文目录**：原文/
- **译文目录**：译文/
- **译文命名**：原名.zh-CN.md（放译文目录）
- **领域配置**：prompts/领域配置/小说文学.md
- **术语表**：术语表.md
- **内容形态**：长篇（委派模式）— 献词 + 51 章（罗马数字 I–LI）+ 尾注，共 53 篇，源文总计约 442KB
...
```
file,size_kb,status
00-dedication.md,1.4,todo
01-birth-of-mangogul.md,4.6,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `01-birth-of-mangogul.md` |
...
> **谁执行**由 `项目说明.md` 的「内容形态」字段决定：本项目为长篇 → **委派模式**，主 agent 派单，子代理逐篇执行以下 9 步。
...
- 严守术语表：遇表内源词必译为指定译法（核心词 **Toy → 玩意**）。新词自行确定统一译法，首次附原文「中文（English）」。
...
共 53 篇，直接用 `translation_queue.csv` 管理（此表省略）。要点：
- `00-dedication.md` 献词（Zima 致读者的戏拟献词）
- `01`–`51` 对应第 I–LI 章（含「试戒」系列插曲、班扎科学院讽刺、婆罗门布道、塞利姆游记、化装舞会等）
- `52-endnotes.md` 尾注（3 条，最短）
