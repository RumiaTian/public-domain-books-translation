...
```
file,size_kb,status
01-dedication.md,0.3,todo
02-advertisement.md,0.9,todo
03-book-1-chapter-01.md,9.2,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `03-book-1-chapter-01.md` |
...
> 本项目为**委派模式**（长篇），由子代理逐篇执行；每篇执行者需先读：本 Plan.md、项目说明.md、术语表.md、prompts/通用翻译引擎.md、prompts/领域配置/小说文学.md、译文/ 下任一已完成 .zh-CN.md（黄金样本，无则跳过），以及前一篇译文末尾 2-3 段（首篇无此项）。
...
- 长文（源文 >50KB）分段处理见通用翻译引擎第七节（本项目最大单章约 37KB，一般无需分段）。
...
共 74 篇（01–02 前置文本 + 6 卷 71 章 + 74 尾注），逐文件清单见 `translation_queue.csv`，卷分布如下：
