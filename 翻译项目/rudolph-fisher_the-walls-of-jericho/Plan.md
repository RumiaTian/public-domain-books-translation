...
> 本计划适配任意整书翻译。复制自 `prompts/翻译计划模板.md`，填入本书信息后即可用通用提示词推进。

---

## 本计划信息

- **项目名称**：耶利哥的城墙（The Walls of Jericho）—— Rudolph Fisher
- **源文目录**：原文/
- **译文目录**：译文/
...
02-jericho.md,0.0,todo
03-chapter-1.md,12.3,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `03-chapter-1.md` |
...
> 本书为「长篇／委派模式」：由子代理逐章执行以下流程。无论谁执行，这 9 步不变。
...
- AAVE 方言对白声音须全文一致（市井口语化汉语，避免书面腔），照黄金样本风格译；详见术语表「翻译策略」。
- 种族／阶级俚语（dickty↔rat、fay、shine/jig/boogy 等）按术语表统一处理。
- 人物/语气/风格全文一致（文学类尤其重要）。
...
由 `translation_queue.csv` 管理（共 35 个文件：卷首民歌 1、献词 1、六部扉页 6、26 章正文、卷末"哈莱姆语词典" 1），此处不另列。
...
| 日期 | 篇目 | 结果 | 备注 |
|------|------|------|------|
| 2026-08-31 | 全书 35 项（卷首2件＋26章＋7间章标题件＋附录词典1件） | 完成 35/35 | 批次1三段并行（86/90/89KB）零[1301]零[1308]；3件数字锚点误报核实无害（309/1500/1619 均在中文段）；术语表旧种子（闪亮/巴伯）勘误，全书夏因/布伯统一；跨段漂移2处终验统一（宝贝·梅里马克、K.M.帮佣丫头）；ch34 哈莱姆语词典词条照搬英文释义照译；间章题名 Jive 定译「花腔」 |
