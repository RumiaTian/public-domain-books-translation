# 翻译计划（Plan.md）— baroness-orczy_el-dorado
...
```
file,size_kb,status
01-foreword.md,5.4,todo
02-part-i.md,0.0,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `03-in-the-th-tre-national.md` |
...
由 `translation_queue.csv` 统一管理（53 篇：01 作者序、02/26/38 三部分隔页、其余为第一部 23 章 + 第二部 11 章 + 第三部 15 章共 49 章，全 todo），此处不重复列表。
