...
- **项目名称**：fitz-hugh-ludlow_the-hashish-eater（《哈希什食客》）
- **源文目录**：原文/
- **译文目录**：译文/
...
```
file,size_kb,status
01-epigraph.md,0.2,todo
02-preface.md,5.6,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `04-the-night-entrance.md` |
...
> **谁执行**由 `项目说明.md` 的「内容形态」字段决定：本项目为长篇，走**委派模式**（子代理逐篇执行下述 9 步，简报见 `WORKFLOW.md`）。
...
- 严守术语表：hashish 统一译「哈希什」、Cannabis Indica 统一「印度大麻」等，遇表内源词必用指定译法。新词自行确定统一译法，首次附原文。
- 章节标题为罗马数字 + 英文名，照「罗马数字 + 中文译名」格式合并标题。
- 拉丁文引语保留原文并附中译/简释（见术语表策略总则）。
- 人物/语气/风格全文一致，照黄金样本风格译。
- 长文（源文 >50KB：`30-labyrinths-and-guiding-threads.md`、`31-ideal-men-and-their-stimulants.md`）分段处理见通用翻译引擎第七节。
...
共 32 篇：前置件 3（epigraph / preface / introduction）+ 正文 25 章（04–28）+ 附录 3（29 notes-on-the-way-upward / 30 labyrinths / 31 ideal-men）+ 尾注 1（32 endnotes）。清单与大小以 `translation_queue.csv` 为准（合计约 573KB）。
