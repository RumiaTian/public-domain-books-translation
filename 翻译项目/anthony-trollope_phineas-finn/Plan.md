...
- **项目名称**：anthony-trollope_phineas-finn（《菲尼亚斯·芬恩》，安东尼·特罗洛普）
- **源文目录**：原文/
- **译文目录**：译文/
...
```
file,size_kb,status
01-phineas-finn-proposes-to-stand-for-loughshane.md,23.0,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `01-phineas-finn-proposes-to-stand-for-loughshane.md` |
...
> **谁执行**由 `项目说明.md` 的「内容形态」字段决定（见 `WORKFLOW.md`「判断内容形态」）：本项目为长篇 → 委派模式，子代理逐篇执行本流程。无论谁执行，这 9 步不变。
...
共 76 篇：01-phineas-finn-proposes-to-stand-for-loughshane.md … 76-conclusion.md（第 1–76 章）。文件名两位数字前缀即阅读顺序。完整清单与状态见 `translation_queue.csv`（权威源）。
