...
- **项目名称**：fritz-leiber_short-fiction（弗里茨·莱伯短篇科幻小说集）
- **源文目录**：原文/
- **译文目录**：译文/
...
```
file,size_kb,status
a-pail-of-air.md,12.3,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `a-pail-of-air.md` |
...
> **谁执行**由 `项目说明.md` 的「内容形态」字段决定：本书为短篇集，走委派模式，由子代理逐篇执行；无论谁执行，9 步不变。
...
- 长文（源文 >50KB，本书为 bullet-with-his-name.md）分段处理见通用翻译引擎第七节。
...
共 25 篇，详见 `translation_queue.csv`（由 `gen_project_files.py` 生成）。
