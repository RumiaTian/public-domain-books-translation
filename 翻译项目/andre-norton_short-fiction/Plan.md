...
```
file,size_kb,status
01-the-people-of-the-crater.md,84.3,todo
02-the-gifts-of-asti.md,30.8,todo
03-all-cats-are-gray.md,14.8,todo
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `01-the-people-of-the-crater.md` |
...
> **谁执行**由 `项目说明.md` 的「内容形态」字段决定：本项目为短篇集，走**委派模式**（子代理逐篇执行下述 9 步）。
...
- **01 篇 84KB 为长文**：按通用翻译引擎第七节分段处理，段间进度自记，单篇不可中途截断交付。
- 01 篇内部分节标题为罗马数字（I–XI），照搬不译。
...
| # | 篇名 | 源文 | 大小 | 状态 |
|---|------|------|------|------|
| 1 | The People of the Crater《陨石坑中的居民》 | 01-the-people-of-the-crater.md | 84.3KB | todo |
| 2 | The Gifts of Asti《阿斯蒂的馈赠》 | 02-the-gifts-of-asti.md | 30.8KB | todo |
| 3 | All Cats Are Gray《灰猫》 | 03-all-cats-are-gray.md | 14.8KB | todo |
...
|------|------|------|------|
| - | - | - | （尚未运行） |
| 2026-08-17 | 全书译完 3/3 | 01 火山口之民 84.3KB（125对块，远古族段落钦定本式庄重语域）/ 02 阿斯蒂的赠礼 30.8KB（31对块）/ 03 猫皆是灰色 14.8KB（13对块），全部 exit 0、0 错配；术语表补 31 条（蜥蜴民/思拉拉/凯普塔/深渊守护者等）。另：检测到并行会话在 译文/_parts 留有 01 篇分段工作文件，与本项目无关未触碰 |
