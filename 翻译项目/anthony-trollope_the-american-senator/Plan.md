# 翻译计划 — anthony-trollope_the-american-senator

---

## 本计划信息

- **项目名称**：anthony-trollope_the-american-senator（特罗洛普《美国参议员》）
- **源文目录**：原文/
- **译文目录**：译文/
- **译文命名**：原名.zh-CN.md（放译文目录）
- **领域配置**：prompts/领域配置/小说文学.md
- **术语表**：术语表.md
- **内容形态 / 执行模式**：长篇 / 委派（子代理逐章执行下方 9 步）
...
```
file,size_kb,status
01-dillsborough.md,15.5,todo
02-the-morton-family.md,14.1,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `01-dillsborough.md` |
...
> 本项目为**委派模式**：主 agent 只当调度员，把每章的 9 步下沉到一次性子代理执行（并发 ≤ 3），调度简报见 `WORKFLOW.md`「委派模式调度循环」。
...
- 标题格式：全部「英文 / 中文」合并（罗马数字章号照搬，如 `## XIII At Bragton / 十三 在布拉格顿`）。
...
全书 80 章（文件名两位数字前缀 = 阅读顺序），完整清单见 `translation_queue.csv`。要点：

| 段落 | 章 | 内容 |
|------|----|------|
| 开局 | 01-11 | 迪尔斯伯勒风物、莫顿/马斯特斯两家世系、俱乐部恩怨、猎狐纠纷 |
| 阿拉贝拉登场 | 12-27 | 特雷福尔母女做客布拉格顿、拉福德府猎狐与攻防 |
| 伦敦线 | 28-33 | 外交部众生相、参议员书信 |
| 迫害与转圜 | 34-50 | 阿拉贝拉迫害拉福德勋爵、乔顿农场、佩尔特里之日 |
| 决裂 | 51-66 | 阿拉贝拉决断、莫顿太太回府、遗嘱与产业 |
| 收束 | 67-80 | 拉福德模范农场、斯克罗比庭审、两场婚礼、参议员演讲两篇、结局 |
