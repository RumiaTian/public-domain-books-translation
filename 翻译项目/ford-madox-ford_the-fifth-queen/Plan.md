# 翻译计划（Plan.md）— 第五王后（The Fifth Queen）
...
```
file,size_kb,status
01-chapter-01.md,25.0,todo
02-chapter-02.md,19.1,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `01-chapter-01.md` |
...
> **谁执行**由 `项目说明.md` 的「内容形态」字段决定（见 `WORKFLOW.md`「判断内容形态」）：本项目为长篇，走**委派模式**，由子代理逐篇执行本流程。无论谁执行，这 9 步不变。
...
全书 3 个分卷页 + 20 章，共 23 个文件（详表见 `translation_queue.csv`）：

| 区段 | 文件 | 内容 |
|------|------|------|
| Part I | `00-part-1-the-coming.md` + `01`–`07-chapter-01..07.md` | 第一部「驾临」，7 章 |
| Part II | `08-part-2-the-house-of-eyes.md` + `09`–`17-chapter-08..16.md` | 第二部「眼目之屋」，9 章 |
| Part III | `18-part-3-the-king-moves.md` + `19`–`22-chapter-17..20.md` | 第三部「国王行动」，4 章 |
