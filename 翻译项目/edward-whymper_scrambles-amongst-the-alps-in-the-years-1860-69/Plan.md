# Plan.md — Scrambles Amongst the Alps in the Years 1860–69

---

## 本计划信息

- **项目名称**：edward-whymper_scrambles-amongst-the-alps-in-the-years-1860-69（中文书名暂定《阿尔卑斯攀登记：1860—1869》）
- **源文目录**：原文/
- **译文目录**：译文/
...
file,size_kb,status
preface.md,3.2,todo
introductory.md,25.1,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `introductory.md` |
...
- 长文（源文 >50KB，如 endnotes.md、subsequent-history 等篇）分段处理见通用翻译引擎第七节。
...
> 完整清单以 `translation_queue.csv` 为准（36 个文件，含正文各章、前言、尾注、插图目录等）。全书结构：Preface → Introductory（第一章）→ 各章按攀登年份与山峰成篇（马特洪峰多次尝试、首登与山难为核心主线）→ 附录（马特洪峰攀登尝试表、冰川学专章等）→ Endnotes 尾注。
