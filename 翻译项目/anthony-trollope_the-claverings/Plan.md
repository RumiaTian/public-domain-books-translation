# 翻译计划（Plan.md）— anthony-trollope_the-claverings
...
```
file,size_kb,status
01-julia-brabazon.md,16.3,todo
02-harry-clavering-chooses-his-profession.md,28.5,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `16-the-rivals.md` |
...
> **谁执行**：本项目内容形态为**长篇**，执行模式**委派**——主 agent 派单，一次性子代理逐篇执行下面 9 步（简报清单见 `WORKFLOW.md`「委派模式调度循环」）。
...
- 长文（源文 >50KB）分段处理见通用翻译引擎第七节（本书各章 ≤30KB，一般无需分段）。
...
全书 48 章（罗马数字 I–XLVIII），按 `translation_queue.csv` 管理（01-julia-brabazon.md … 48-conclusion.md，数字前缀即阅读顺序），此表省略。
