# 翻译计划 — 《公爵的儿女》（The Duke's Children）

## 本计划信息

- **项目名称**：anthony-trollope_the-dukes-children（《公爵的儿女》，Anthony Trollope，1880）
- **源文目录**：原文/
- **译文目录**：译文/
- **译文命名**：原名.zh-CN.md（放译文目录）
- **领域配置**：prompts/领域配置/小说文学.md
- **术语表**：术语表.md
- **内容形态 / 执行模式**：长篇 / 委派（80 章，由子代理逐章执行下方 9 步）
...
```
file,size_kb,status
01-when-the-duchess-was-dead.md,16.5,todo
02-lady-mary-palliser.md,19.6,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 两位数字前缀 + 章名 kebab-case |
...
> **谁执行**由 `项目说明.md` 的「内容形态」字段决定：本项目为**委派模式**，每章由子代理逐篇执行本流程；主 agent 只做调度（见 `WORKFLOW.md`「委派模式调度循环」）。
...
全书 80 章，按 `translation_queue.csv`（01–80 数字前缀）顺序推进，此处不重复罗列。
