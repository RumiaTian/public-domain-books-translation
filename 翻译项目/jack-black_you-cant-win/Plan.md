...
- **项目名称**：jack-black_you-cant-win（《你赢不了》）
- **源文目录**：原文/
- **译文目录**：译文/
...
```
file,size_kb,status
00-acknowledgments.md,0.3,todo
00-foreword.md,8.0,todo
chapter-01.md,8.4,todo
...
chapter-24.md,36.0,todo
```
...
> **谁执行**由 `项目说明.md` 的「内容形态」字段决定：本项目为长篇 → **委派模式**，主 agent 只当调度员，把每章的 9 步下沉到一次性子代理执行（并发 ≤ 3），见 `WORKFLOW.md`「委派模式调度循环」与「子代理简报」。
...
- 严守术语表：遇表内源词（尤其黑话表）必译为指定译法。新词自行确定统一译法，首次附原文「中文（English）」，并回填术语表。
- 罗马数字章题照搬：标题按「I / 一」合并格式。
- 人物口吻、黑话色彩、方言对白全文一致，照黄金样本风格译。
...
- 标题格式：全部「英文 / 中文」合并。
- 术语一致：抽查 5 个黑话词（yegg/hop/mark/stir/snitch 等），全文译法统一。
- 不满足则改到满足。
...
共 26 件：献词 + 前言 + 24 章，约 620KB。以 `translation_queue.csv` 为准（顺序 = 阅读顺序：00-acknowledgments → 00-foreword → chapter-01 … chapter-24）。
