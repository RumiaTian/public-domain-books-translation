...
- **项目名称**：anthony-trollope_can-you-forgive-her（《你能原谅她吗？》，安东尼·特罗洛普）
- **源文目录**：原文/
- **译文目录**：译文/
...
```
file,size_kb,status
01-mr-vavasor-and-his-daughter.md,15.4,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `01-mr-vavasor-and-his-daughter.md` |
...
> **谁执行**由 `项目说明.md` 的「内容形态」字段决定：本 项目为**长篇 → 委派模式**，子代理逐章执行本 9 步流程。
...
- 严守术语表：遇表内源词必译为指定译法。新词自行确定统一译法，首次附原文「中文（English）」，并补入术语表。
- 人物/语气/风格全文一致（文学类尤其重要），照黄金样本风格译。
- 长文（源文 >50KB）分段处理见通用翻译引擎第七节。本书单章 12–34KB，一般无需分段。
...
共 81 篇：01-mr-vavasor-and-his-daughter.md … 80-the-story-is-finished-within-the-halls-of-the-duke-of-omnium.md（第 1–80 章）+ 81-endnotes.md（作者尾注）。文件名两位数字前缀即阅读顺序。完整清单与状态见 `translation_queue.csv`（权威源）。
