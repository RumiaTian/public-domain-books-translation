...
- **项目名称**：metta-victor_the-dead-letter（《死信》 The Dead Letter, Metta Victoria Victor, 1866）
- **源文目录**：原文/
- **译文目录**：译文/
...
```
file,size_kb,status
00-part-i.md,0.0,todo
01-the-letter.md,3.4,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `01-the-letter.md` |
...
> **谁执行**由 `项目说明.md` 的「内容形态」字段决定：本项目为长篇 → **委派模式**，主 agent 调度、子代理逐章执行以下 9 步。
...
- 委派模式子代理简报另含：前一篇末尾 2-3 段原文+译文（取不到时用黄金样本替代）。
...
> 完整清单见 `translation_queue.csv`（25 个文件，队列顺序=阅读顺序）。结构：Part I（章 I–XIII）+ Part II（章号重新从 I 起，前缀 15-24）。00/14 两个文件为分部扉页。

| # | 篇名（文件） | 大小 | 状态 |
|---|------|------|------|
| 1 | 00-part-i.md | 0.0KB | todo |
| 2 | 01-the-letter.md | 3.4KB | todo |
| 3 | 02-events-of-a-night.md | 19.2KB | todo |
| … | （详见 translation_queue.csv） | … | todo |
| 25 | 24-the-new-life.md | 6.9KB | todo |
...
| 日期 | 篇目 | 结果 | 备注 |
|------|------|------|------|
| 2026-08-17 | 建项目 | 完成 | epub 提取 25 文件（23 章 + 2 分部扉页），领域=小说文学，形态=长篇，模式=委派 |
