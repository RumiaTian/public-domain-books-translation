...
- **项目名称**：班卓（Banjo: A Story without a Plot）—— Claude McKay
- **源文目录**：原文/
- **译文目录**：译文/
...
02-the-ditch.md,24.9,todo
...
```

| 字段 | 含义 | 取值 |
|------|------|------|
| `file` | 源文文件名（在 `原文/` 目录下） | 如 `02-the-ditch.md` |
...
- 方言对白（AAVE／法语／普罗旺斯）声音须全文一致，照黄金样本风格译；详见术语表「翻译策略」。
...
由 `translation_queue.csv` 管理（共 29 个文件：1 篇献词、3 个分部扉页、25 章正文），此处不另列。
